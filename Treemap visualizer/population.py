...
class PopulationTree(AbstractTree):
    ...
    subtrees: Optional[List[PopulationTree]]
...
def _load_data() -> List[PopulationTree]:
    ...
    for country in country_populations:
        ...
        tree = PopulationTree(False, country, [], pop)
        countries.append(tree)

    for region in regions:
        subtrees = []
        pop = 0
        for country in countries:
            pop = country.data_size
            if country.root in regions[region] and pop is not None:
                subtrees.append(country)
        tree = PopulationTree(False, region, subtrees, pop)
        trees.append(tree)
